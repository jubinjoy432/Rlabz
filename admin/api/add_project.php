...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['faculty_photo']) && $_FILES['faculty_photo']['error'] === UPLOAD_ERR_OK) {
        $ext = strtolower(pathinfo($_FILES['faculty_photo']['name'], PATHINFO_EXTENSION));
        if (in_array($ext, $allowed)) {
            $fname = uniqid('faculty_') . '.' . $ext;
            if (move_uploaded_file($_FILES['faculty_photo']['tmp_name'], $uploadDir . $fname)) {
                $faculty_photo = 'uploads/' . $fname;
            }
        }
    }
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO projects (slug, title, year, short_description, description, objectives, problem_statement, expected_outcome, tech_stack, key_features, image_path, thumbnail_path, department, batch, faculty_name, faculty_designation, faculty_photo, category, project_type, duration, status, github_link, demo_link, poster_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            ...
            $department, $batch, $faculty_name, $faculty_designation,
            $faculty_photo, $category, $project_type, $duration,
        ...
        if (isset($_POST['member_names']) && is_array($_POST['member_names'])) {
            ...
            $stmtMember = $pdo->prepare("INSERT INTO project_members (project_id, name, photo_path, role) VALUES (?, ?, ?, ?)");
            ...
            foreach ($member_names as $index => $m_name) {
                ...
                $stmtMember->execute([$project_id, $m_name, $m_photo_path, $m_role]);
            }
        }
